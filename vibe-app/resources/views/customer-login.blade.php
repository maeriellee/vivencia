...
<head>
    ...
    <title>Customer Login | Vivencia</title>
...
<body class="vivencia-shell">
    <main class="grid min-h-screen place-items-center px-4">
        <form class="vivencia-card w-full max-w-md p-7" method="POST" action="{{ route('customer.authenticate') }}">
            @csrf
            <img class="mx-auto h-20 w-20 rounded-full object-contain" src="/images/menu/vivencia-logo.jpeg" alt="Vivencia logo">
            <h1 class="mt-5 text-center font-[Epilogue] text-3xl font-extrabold">Customer Dashboard</h1>
            <p class="mt-2 text-center text-on-surface-variant">Private access for existing Vivencia customers.</p>
            ...
            <button class="vivencia-button mt-6 w-full" type="submit">Log in</button>
            <a class="mt-4 block text-center text-sm font-bold text-primary underline" href="/menu">Start an order instead</a>
        </form>
    </main>
</body>
</html>
